...
start_time=$(date +%s)
conda run -n pagerank python rank_normalized_feature_reduction.py "$NUMBER_OF_FEATURES" "$MAPPING_FILE" "$BUCKET_NAME" "$SUBLOCATION" "$PPR_FEATURE_LOCATION" "$SAVE_PATH" "$SAVE_FILENAME" "$CHECK_ANY_SAVED_NODES" "$NODE_TYPE" "NODE_TYPE_SEPERATOR" >> logs/rank_normalized_feature_reduction.log 2>&1 &
wait
end_time=$(date +%s)
