...
PPR_FEATURE_MAP_FILELOCATION="spoke35M/spoke35M_compound_pruned_version_converged_ppr/spoke35M_ppr_features.csv"
...
start_time=$(date +%s)
conda run -n pagerank python extract_all_bacteria_for_bcmm_compounds.py "$GRAPH_PATH" "$MAPPING_FILE" "$BACTERIA_FILE" "$BUCKET_NAME" "$PPR_FILE_LOCATION" "$PPR_FEATURE_MAP_FILELOCATION" "$SAVE_LOCATION" "$SAVE_NAME" "$NCORES" "$NODE_TYPE_SEPARATOR" >> logs/run_bacterial_extraction_from_bcmm_compounds.log 2>&1 &
wait
end_time=$(date +%s)
